...
def time_tf_run(session, target, info_str):
    ...
    sd = math.sqrt(vr)
    print('%s: %s across %04d steps, %.3f +/- %.3f sec / batch' \
          % (datetime.now(), info_str, num_batches, mn, sd))


def run_benchmark():
    with tf.Graph().as_default():
        image_size = 24  # original 224, modified as 24 here for speed
